...
function renderBubble(idea, state) {
  ...
  return `
    <div class="idea-bubble ${bubbleClass} ${isMerged ? 'merged-bubble' : ''}"
      ...
      <div class="confidence-ring" style="background: conic-gradient(var(--color-bubble-${idea.color_variant}) ${idea.confidence}%, transparent ${idea.confidence}%); border-color: var(--color-bubble-${idea.color_variant}); color: var(--color-text-primary);">
